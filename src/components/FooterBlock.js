import { Link } from "react-router-dom";
// --- SVG Icons for Social Media ---
const LinkedInIcon = () => (
    <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
        <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
    </svg>
);

const GithubIcon = () => (
    <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
    </svg>
);

const FooterBlock = () => {
  return (
    <footer className="bg-yellow-600 text-zinc-200 rounded-2xl p-8 md:p-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            
            {/* Column 1: Name and Bio */}
            <div className="md:col-span-1">
                <h3 className="font-bold text-xl uppercase">Aakanksha Sonawane.</h3>
                <p className="text-sm mt-2 text-zinc-300">
                    A Business Intelligence & Data Analyst transforming data into actionable insights to support strategic decisions.
                </p>
                <p className="text-xs text-orange-800 mt-8">
                    Aakanksha Sonawane - Copyright &copy; {new Date().getFullYear()}
                </p>
            </div>

            {/* Column 2: Sitemap */}
            <div className="md:col-span-1">
                <h4 className="font-bold uppercase text-orange-800 text-sm mb-4">Sitemap</h4>
                <ul className="space-y-2 text-sm">
                    <li>
                    <Link to="/" className="hover:text-orange-800 ">Home</Link>
                    </li>
                     <li>
                    <Link to="/about" className="hover:text-orange-800 ">About</Link>
                    </li>
                    <li>
                    <Link to="/skills" className="hover:text-orange-800 ">Skills</Link>
                    </li>
                    <li>
                    <Link to="/projects" className="hover:text-orange-800 ">Projects</Link>
                    </li>
                   
                </ul>
            </div>

            {/* Column 3 & 4: Socials and Contact */}
            <div className="md:col-span-2 flex flex-col justify-between items-start md:items-end">
                <div>
                    <h4 className="font-bold uppercase text-sm mb-4 text-orange-800 text-left md:text-right">Follow me on</h4>
                    <div className="flex space-x-4">
                        <a href="https://www.linkedin.com/in/aakanksha-sonawane/" aria-label="LinkedIn" className="hover:text-white"><LinkedInIcon /></a>
                        
                        <a href="https://github.com/Aakanksha2412" aria-label="GitHub" className="hover:text-white"><GithubIcon /></a>
                    </div>
                </div>
               <Link
                    to="/contact"
                    className="border-2 border-zinc-300 rounded-lg px-8 py-3 mt-8 md:mt-0 font-bold text-sm hover:bg-zinc-200 hover:text-orange-900 transition-colors text-center"
                    >
                    CONTACT ME
                </Link>

            </div>

        </div>
    </footer>
  );
};

export default FooterBlock;
